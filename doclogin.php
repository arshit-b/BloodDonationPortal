...
<?php
	...
	//Checking is user existing in the database or not
        $query = "SELECT * FROM `doc_register` WHERE username='$username' and password='".md5($password)."'";
		$result = mysqli_query($con,$query) or die(mysql_error());
		$rows = mysqli_num_rows($result);
        if($rows==1){
			$_SESSION['username'] = $username;
			header("Location: filterdonor.php"); // Redirect user to index.php
            }else{
				echo "<script type='text/javascript'>alert('Username or password is incorrect!!')
                document.location.href = 'doclogin.php'</script>";
				}
    }else{
    }
?>
